...
const Fish: React.FC = () => {
  ...
  // --- Single velocity vector instance ---
  ...
  // --- Tail segments positions ---
  const tailPositions = useRef<THREE.Vector3[]>([])
  // TODO: Consider converting tail segments to an InstancedMesh for reduced draw calls
  ...
  // --- Helper: Compute a smooth intended heading for the head ---
      ...
      words.forEach((word, index) => {
        setTimeout(() => {
          setDisplayText(prev => {
            ...
            return newText.padEnd(fullText.length, ' ');
          });
